...
const Cart = ({ product, setProduct }) => {
  ...
  return (
    <div className='lg:w-[60%] w-[90%] mx-auto py-10 space-y-8'>
      ...
      {product.length === 0 ? (
        <p className="text-center text-gray-500 text-2xl font-semibold py-10">No products added to Cart</p>
      ) : (
        <div className="space-y-4">
          {product.map((item) => (
            <div 
              key={item.id} // Added unique key
              className='border-2 border-purple-400 hover:px-6 transition-[0.35s] p-5 rounded-3xl flex items-center justify-between'
            >
